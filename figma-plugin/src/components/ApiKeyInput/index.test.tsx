...
describe('ApiKeyInput', () => {
  ...
  it('コンポーネントが正しくレンダリングされる', () => {
    ...
    render(<ApiKeyInput value="" onChange={mockOnChange} isValid={mockIsValid} />);
  ...
  it('入力値が正しく表示される', () => {
    mockIsValid.mockReturnValue(true);
    render(<ApiKeyInput value="sk-ant-test" onChange={mockOnChange} isValid={mockIsValid} />);
  ...
  it('表示切替後はaria-labelが変わる', async () => {
    ...
    expect(screen.getByRole('button', { name: 'API Keyを非表示' })).toBeInTheDocument();
  });
  ...
  it('空の値の場合はバリデーションエラーが表示されない', () => {
    ...
    expect(
      screen.queryByText('API Keyは "sk-ant-" で始まる必要があります')
    ).not.toBeInTheDocument();
    expect(screen.getByText('このAPI Keyはデバイスにローカルに保存されます')).toBeInTheDocument();
  });

  it('値が空でない場合でisValidがfalseの場合はエラーが表示される', () => {
    mockIsValid.mockReturnValue(false);
    render(<ApiKeyInput value="invalid-key" onChange={mockOnChange} isValid={mockIsValid} />);

    expect(screen.getByText('API Keyは "sk-ant-" で始まる必要があります')).toBeInTheDocument();
    expect(
  ...
  it('エラーメッセージがrole="alert"を持つ', () => {
    ...
    const errorMessage = screen.getByText('API Keyは "sk-ant-" で始まる必要があります');
