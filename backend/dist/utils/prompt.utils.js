...
exports.formatFigmaDataForEvaluation = formatFigmaDataForEvaluation;
exports.buildStylesApplicationMap = buildStylesApplicationMap;
exports.extractJsonFromResponse = extractJsonFromResponse;
exports.extractNodeHierarchyPath = extractNodeHierarchyPath;
exports.getJsonSchemaTemplate = getJsonSchemaTemplate;
exports.getNodeIdInstructions = getNodeIdInstructions;
exports.buildSystemPromptSuffix = buildSystemPromptSuffix;
exports.getNodeIdReminder = getNodeIdReminder;
exports.buildColorContrastMap = buildColorContrastMap;
 ...
 */
function formatFigmaDataForEvaluation(data) {
 ...
 * ノードを再帰的に整形
 */
function formatNodeRecursive(node, depth, visited = new Set(), maxDepth = 10) {
    ...
    if (node.fills && node.fills.length > 0) {
        node.fills.forEach((fill, index) => {
            if (fill.type === 'SOLID' && fill.color) {
                const color = (0, accessibility_1.rgbToHex)(fill.color.r, fill.color.g, fill.color.b);
    ...
    if (node.strokes && node.strokes.length > 0 && node.strokeWeight) {
        node.strokes.forEach((stroke, index) => {
            if (stroke.type === 'SOLID' && stroke.color) {
                const color = (0, accessibility_1.rgbToHex)(stroke.color.r, stroke.color.g, stroke.color.b);
 ...
 */
function buildStylesApplicationMap(nodeData, stylesData) {
    if (!stylesData) {
 ...
 * ノードツリーからスタイル使用状況を収集
 */
function collectStyleUsage(node, stylesData, stats = {
    variablesUsed: [],
    textStylesUsed: [],
 ...
 */
function extractNodeHierarchyPath(data, targetNodeId, currentPath = [], visited = new Set()) {
    // 循環参照チェック
 ...
 */
function getNodeIdInstructions() {
...
function extractTextAndBackgroundColors(node, parentBackgroundColor) {
    ...
    if (node.type === 'TEXT' && node.fills && node.fills.length > 0) {
        ...
        if (fill.type === 'SOLID' && fill.color && (fill.opacity === undefined || fill.opacity > 0.1)) {
            textColor = (0, accessibility_1.rgbToHex)(fill.color.r, fill.color.g, fill.color.b);
        }
    }
    // テキストノード以外の場合、背景色を取得
    if (node.type !== 'TEXT') {
        if (node.fills && node.fills.length > 0) {
            ...
            if (fill.type === 'SOLID' &&
                ...
                backgroundColor = (0, accessibility_1.rgbToHex)(fill.color.r, fill.color.g, fill.color.b);
            }
        }
 ...
 */
function isFullyContainedBy(textBox, backgroundBox) {
    return (backgroundBox.x <= textBox.x &&
...
function extractBackgroundColor(node) {
    ...
    if (node.fills && Array.isArray(node.fills) && node.fills.length > 0) {
        const solidFill = node.fills.find((fill) => fill.type === 'SOLID' && fill.color !== undefined);
        if (solidFill) {
            const opacity = solidFill.opacity ?? 1;
            if (opacity > 0) {
                const { r, g, b } = solidFill.color;
                return (0, accessibility_1.rgbToHex)(r, g, b);
            }
        }
    }
    return undefined;
 ...
 * @returns 背景色、見つからない場合はundefined
 */
function findBackgroundColorWithHierarchy(rootNode, textNode, currentParent, depth = 0, visited = new Set()) {
    // 安全対策: 最大深度チェック
    if (depth > 20) {
        (0, debug_1.log)('warn', `Max depth reached for: ${textNode.name}, Depth: ${depth}`, '[Hierarchical Search]');
        return undefined;
    }
    // 親ノードが存在しない場合は終了
    if (!currentParent) {
        (0, debug_1.log)('debug', 'No parent found, ending search', '[Hierarchical Search]');
        return undefined;
    }
    // 安全対策: 循環参照チェック
    if (visited.has(currentParent.id)) {
        (0, debug_1.log)('warn', `Circular reference detected: Node: ${currentParent.name} (ID: ${currentParent.id})`, '[Hierarchical Search]');
        return undefined;
    }
    visited.add(currentParent.id);
    (0, debug_1.log)('debug', `Level ${depth}: Searching for: ${textNode.name}, Current parent: ${currentParent.name} (ID: ${currentParent.id})`, '[Hierarchical Search]');
    // [Step 1] 兄弟要素から検索
    if (currentParent.children) {
        (0, debug_1.log)('debug', `Step 1 (Level ${depth}): Searching siblings, Siblings count: ${currentParent.children.length}`, '[Hierarchical Search]');
        const siblingBackground = findSiblingBackgroundColor(currentParent.children, textNode);
        if (siblingBackground) {
            (0, debug_1.log)('info', `✓ Found sibling background at depth ${depth}: Background: ${siblingBackground}, Parent: ${currentParent.name}`, '[Hierarchical Search]');
            return siblingBackground;
        }
    }
    // [Step 2] 親要素自体の背景色を確認
    (0, debug_1.log)('debug', `Step 2 (Level ${depth}): Checking parent's own background`, '[Hierarchical Search]');
    const parentBackground = extractBackgroundColor(currentParent);
    if (parentBackground) {
        (0, debug_1.log)('info', `✓ Found parent's own background at depth ${depth}: Background: ${parentBackground}, Parent: ${currentParent.name}`, '[Hierarchical Search]');
        return parentBackground;
    }
    // [Step 3] ルートノードに到達した場合は終了
    if (isRootNode(currentParent, rootNode)) {
        (0, debug_1.log)('debug', 'Reached root, no background found', '[Hierarchical Search]');
        return undefined;
    }
    // [Step 4] 祖父母ノードを取得
    const grandparent = findParentNode(rootNode, currentParent);
    if (!grandparent) {
        (0, debug_1.log)('debug', `No grandparent found for: ${currentParent.name}`, '[Hierarchical Search]');
        return undefined;
    }
    // [Step 5] 再帰的に上位階層へ
    (0, debug_1.log)('debug', `Moving up to level ${depth + 1}: Grandparent: ${grandparent.name} (ID: ${grandparent.id})`, '[Hierarchical Search]');
 ...
 */
function findSiblingBackgroundColor(siblings, referenceNode) {
    // 座標ベースの検出を試みる（テキストノードに座標情報がある場合）
    if (referenceNode?.absoluteBoundingBox) {
        const textBox = referenceNode.absoluteBoundingBox;
        (0, debug_1.log)('debug', `Text box: x=${textBox.x}, y=${textBox.y}, w=${textBox.width}, h=${textBox.height}`, '[Sibling Search]');
        // Z-orderを考慮して逆順で探索（最も手前から）
        for (let i = siblings.length - 1; i >= 0; i--) {
            const sibling = siblings[i];
            // スキップ条件のデバッグログ
            if (sibling.type === 'TEXT') {
                (0, debug_1.log)('debug', `Skip TEXT: ${sibling.name}`, '[Sibling Search]');
                continue;
            }
            if (!sibling.fills || sibling.fills.length === 0) {
                (0, debug_1.log)('debug', `Skip no fills: ${sibling.name}`, '[Sibling Search]');
                continue;
            }
            const fill = sibling.fills[0];
            if (fill.type !== 'SOLID' || !fill.color) {
                (0, debug_1.log)('debug', `Skip non-SOLID fill: ${sibling.name}`, '[Sibling Search]');
                continue;
            }
            if (fill.opacity !== undefined && fill.opacity <= 0.1) {
                (0, debug_1.log)('debug', `Skip low opacity: ${sibling.name}`, '[Sibling Search]');
                continue;
            }
            // サイズフィルタリング
            if (!sibling.absoluteBoundingBox) {
                (0, debug_1.log)('debug', `Skip no boundingBox: ${sibling.name}`, '[Sibling Search]');
                continue;
            }
            const siblingBox = sibling.absoluteBoundingBox;
            const siblingWidth = siblingBox.width;
            const siblingHeight = siblingBox.height;
            (0, debug_1.log)('debug', `Checking: ${sibling.name} (${sibling.type}), x=${siblingBox.x}, y=${siblingBox.y}, w=${siblingWidth}, h=${siblingHeight}`, '[Sibling Search]');
            // 幅または高さが10px以下の要素は装飾要素としてスキップ
            if (siblingWidth <= 10 || siblingHeight <= 10) {
                (0, debug_1.log)('debug', `Skip too small: ${sibling.name}`, '[Sibling Search]');
                continue;
            }
            ...
            if (siblingArea < textArea * 0.1) {
                (0, debug_1.log)('debug', `Skip too small area: ${sibling.name}, siblingArea=${siblingArea}, textArea=${textArea}, ratio=${siblingArea / textArea}`, '[Sibling Search]');
                continue;
            }
            // 完全包含チェック
            const isContained = isFullyContainedBy(textBox, siblingBox);
            (0, debug_1.log)('debug', `Containment check: ${sibling.name}, isContained=${isContained}`, '[Sibling Search]');
            if (isContained) {
                const bgColor = (0, accessibility_1.rgbToHex)(fill.color.r, fill.color.g, fill.color.b);
                (0, debug_1.log)('info', `✓ Found background: ${sibling.name}, color=${bgColor}`, '[Sibling Search]');
                return bgColor;
            }
        }
        // 座標情報がある場合、完全包含する背景が見つからなければ
        // 親要素の背景色にフォールバック（undefinedを返す）
        (0, debug_1.log)('warn', 'No matching sibling found, returning undefined', '[Sibling Search]');
        return undefined;
    }
    ...
    for (const sibling of siblings) {
        if (sibling.type !== 'TEXT' && sibling.fills && sibling.fills.length > 0) {
            ...
            if (fill.type === 'SOLID' && fill.color) {
                ...
                if (referenceNode?.absoluteBoundingBox) {
                    const textArea = referenceNode.absoluteBoundingBox.width * referenceNode.absoluteBoundingBox.height;
                    const siblingArea = siblingWidth * siblingHeight;
                    // 兄弟要素の面積がテキストの10%未満の場合はスキップ
                    if (siblingArea < textArea * 0.1)
                        continue;
                }
                return (0, accessibility_1.rgbToHex)(fill.color.r, fill.color.g, fill.color.b);
            }
        }
    }
    return undefined;
 ...
 */
function collectTextColorPairs(node, rootNode, parentBackgroundColor, parentNode, results = [], visited = new Set(), maxResults = 100) {
    // 最大件数に達したら早期終了
    ...
    visited.add(node.id);
    const { textColor, backgroundColor, hasExplicitNoFill } = extractTextAndBackgroundColors(node, parentBackgroundColor);
    // テキストノードの場合
    if (node.type === 'TEXT' && textColor) {
        let finalBackgroundColor;
        // TEXTノードの場合、階層的に背景色を探す
        (0, debug_1.log)('debug', `Starting hierarchical search for: ${node.name} (ID: ${node.id})`, '[Background Detection]');
        // [Step 1] 階層的検索を実行
        finalBackgroundColor = findBackgroundColorWithHierarchy(rootNode, node, parentNode);
        if (finalBackgroundColor) {
            (0, debug_1.log)('info', `✓ Found via hierarchical search: ${finalBackgroundColor}`, '[Background Detection]');
        }
        else {
            (0, debug_1.log)('warn', `Hierarchical search failed for: ${node.name}, No background color found in hierarchy`, '[Background Detection]');
        }
        // 【新規】異常値検出: 文字色と背景色が同一
        if (finalBackgroundColor && textColor === finalBackgroundColor) {
            (0, debug_1.log)('warn', `TEXT node has identical text and background color: Node: ${node.name} (ID: ${node.id}), Color: ${textColor}`, '[Contrast Anomaly]');
            // デバッグモード時の詳細ログ
            (0, debug_1.log)('debug', 'Node data:', '[Contrast Anomaly]', {
                name: node.name,
                id: node.id,
                type: node.type,
                fills: node.fills,
            ...
            if (parentBackgroundColor && parentBackgroundColor !== textColor) {
                ...
                (0, debug_1.log)('info', `Using parent background: ${parentBackgroundColor}`, '[Contrast Anomaly]');
            }
        }
        // 背景色が見つかった場合のみコントラスト比を計算
        if (finalBackgroundColor && results.length < maxResults) {
            try {
                const contrastResult = (0, accessibility_1.calculateWCAGContrast)({
                    color1: textColor,
                ...
                (0, debug_1.log)('error', `Failed to calculate contrast for node ${node.name}:`, undefined, error);
            }
        }
    }
 ...
 */
function groupByColorCombination(contrastInfos) {
 ...
 */
function buildColorContrastMap(data, maxItems = 100) {
 ...
 */
function extractTextNodes(node, results = [], visited = new Set()) {
    // 循環参照チェック
    if (visited.has(node.id)) {
        return results;
    }
