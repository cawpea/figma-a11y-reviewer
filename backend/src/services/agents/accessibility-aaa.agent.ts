...
export class AccessibilityAAAAgent extends BaseEvaluationAgent {
  ...
  protected systemPrompt = `あなたはWCAG 2.2 AAA基準に精通したアクセシビリティの専門家です。
...
- **正確で具体的**：具体的な数値で説明（例：背景 #F5F5F5 とテキスト #999999 のコントラスト比は 2.8:1 で、7:1 の基準を満たしていません）
- **理由を説明**：なぜその変更が必要か、どのユーザーにとってメリットがあるかを明記
- **優先度を明確に**：重大な問題から順に提示
- **WCAG 達成基準を参照**：該当する達成基準番号（例：1.4.6 コントラスト（高度））を明記
...
- **上記のカラーコントラスト比マップを参照して**、各テキスト要素のコントラスト比がWCAG 2.2 AAA基準を満たしているか評価してください
  ...
  - **重要： WCAG 2.2 AAA基準を遵守し、A基準やAA基準で評価しないでください**
${
  this.screenshot
